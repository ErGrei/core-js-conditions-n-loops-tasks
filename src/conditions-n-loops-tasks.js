...
function convertToRomanNumerals(num) {
  let res = '';
  let value = num;
  const arabicNumeral = [10, 9, 5, 4, 1];
  const romanNumeral = ['X', 'IX', 'V', 'IV', 'I'];
  for (let i = 0; i < romanNumeral.length; i += 1) {
    while (value >= arabicNumeral[i]) {
      res += romanNumeral[i];
      value -= arabicNumeral[i];
    }
  }
...
function isContainNumber(num, digit) {
  const stringToCheck = String(num);
  const value = String(digit);
  for (let i = 0; i < stringToCheck.length; i += 1) {
    if (stringToCheck[i] === value) {
      return true;
    }
  }
  return false;
